...
public class SpeechManager : MonoBehaviour {
    ...
    //    var waitForClipRemainingTime = new WaitForSeconds(source.timesource.time);
    ...
    public void SpeakWithSDKPlugin(string message)
    {
        ...
        UnityDispatcher.InvokeOnAppThread(() => {
            StartCoroutine(WaitAndPlayRoutineSDK(Speaking, message));
        });        
    }

    private IEnumerator WaitAndPlayRoutineSDK(Task<SpeechSynthesisResult> speakTask, string message)
    {
        // Yield control back to the main thread as long as the task is still running
        while (!speakTask.IsCompleted)
        {
